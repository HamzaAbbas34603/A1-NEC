...
def main_online_BP(params):
    ...
    errors = []
    errors1 = []
    outFile = open('outputs3/errors_{}.csv'.format(params),'w',encoding='utf-8')
    for epoch in range(0,epochs):
        error = error1 = 0
        treated = []
        for pat in range(0,trainset_size-validation_size):
...
def predict_BP(x_test):
    ...
    for x in x_test:
        ...
        y_pred_scaled.append(np.copy(y_pred))
    return y_pred_scaled

def test_algorithm(y_pred,s_min,s_max, output_file):
    ...
    for i in range(len(y_test)):
        ...
        for j in range(0,dataset.no):
            y += descale_y_value( y_pred[i][j], j, s_min, s_max )
            z += descale_y_value( y_test_scaled[i][j], j, s_min, s_max )
        output.append(y)
        outFile.write("{}, {}, {}\n".format(z, y, abs(z - y)))
        aux1 += abs(z - y)
        aux2 += z
...
def find_best_params(train=False):
    ...
    for item in f.readlines()[1:]:
        try:
            item = item.replace('\n','').split(',')
            h = open('outputs3/errors_{}_0.csv'.format(item[0]))
            last = h.readlines()[-1]
...
def plot_error(id):
    h = open('outputs3/errors_{}.csv'.format(id))
    errors = []
    errors1 = []
    epochs = []
...
def order_best_params():
    ...
    for err in errors[:10]:
        ...
        qt = 0
        for i in range(0,10):
            h = open('outputs3/errors__train_{}_{}.csv'.format(err.get('id'),i))
            last = h.readlines()[-1]
            qt += float(last.replace('\n','').split(',')[1])
            qv += float(last.replace('\n','').split(',')[2])
            h.close()
...
def main():
    ...
    y_pred_scaled = predict_BP(dataset.xtable[trainset_size:])
    y_pred,result = test_algorithm(y_pred_scaled, s_min, s_max, on_test)
    ...
    regr.fit(dataset.xtable[:trainset_size], dataset.ytable[:trainset_size])
    stop = timeit.default_timer()
    print('#####  MLR  #####\tTime (s): ', round(stop - start,2))
    y_pred_scaled = regr.predict(dataset.xtable[trainset_size:])
    y_pred,result = test_algorithm(y_pred_scaled, s_min, s_max, mlr_test)
...
mlr_test = "mlt_test.csv"
activation = epochs = alpha = n = alpha = trainset_size_perc = dataset_name = s_max = s_min = None
read_params()
...
# main()
run_all_params(start=154,end=157)
# find_best_params()
